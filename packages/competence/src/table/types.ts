...
export type TableAggregation = 'sum' | 'min' | 'max' | 'mean' | 'count' | 'uniqueCount'
...
export interface TableRow<T> {
  ...
  grouping?: { columnId: string; value: unknown }
...
export interface TableSortState { columnKey: string; order: TableSortOrder }
export interface TablePaginationState { current: number; pageSize: number }
...
export interface TableState {
  ...
  columnPinning: { start: readonly string[]; end: readonly string[] }
...
export interface TableRowSelection<T> {
  ...
  onChange?: (keys: readonly TableKey[], records: readonly T[], info: { type: TableSelectionAction }) => void
...
export interface TableFeature<T> {
  ...
  transform: (model: TableRowModel<T>, context: { state: TableState; columns: readonly TableColumn<T>[] }) => readonly TableRow<T>[]
}
export interface TableConfig<T> {
  ...
  onStateChange?: (next: TableState, info: { action: TableAction }) => void
  ...
  sortDirections?: readonly (TableSortOrder | null)[]
  onChange?: (pagination: TablePaginationState & { total?: number }, filters: TableFiltersState,
    sorter: TableSortState | readonly TableSortState[] | null, extra: TableChangeInfo<T>) => void
  features?: readonly TableFeature<T>[]
  editing?: TableEditConfig<T>
  virtual?: { estimateRowHeight?: number; overscan?: number }
}
