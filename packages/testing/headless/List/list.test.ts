...
import { createList, type ListScrollToConfig } from '../../../competence/src/list'

const makeItems = (n: number) =>
  Array.from({ length: n }, (_, i) => ({ id: i, name: `item-${i}` }))

const setup = (items: { id: number; name: string }[], opts?: Parameters<typeof createList<{ id: number; name: string }>>[0]) =>
  createList({
    items: () => items,
    rowKey: (item) => item.id,
    ...opts,
...
describe('createList — heights & prefix sums', () => {
  ...
  it('keeps measured heights through append-only growth (infinite loading)', () => {
    ...
    createRoot(() => {
      ...
      const list = createList({ items: () => items(), rowKey: i => i.id })
      flush()
