...
import { createImageGroup } from '../../../competence/src/imageGroup'
...
// 缺失 src 的成员仍占据索引位置，sources 类型必须允许 undefined，不能过滤导致预览错位。
it('[image-group.sources.optional] 缺失图片地址保持成员索引', () => {
  let dispose = () => {}
  try {
    createRoot(cleanup => {
      dispose = cleanup
      const group = createImageGroup()
      group.register(undefined); group.register('second.png'); flush()
      expect(group.sources()).toEqual([undefined,'second.png'])
      group.openAt(1); flush()
      expect(group.currentSrc()).toBe('second.png')
    })
  } finally { dispose() }
})
