...
describe('tree drag/drop', () => {
  ...
  it('blocks self, descendants and disabled rows', () => createRoot(dispose => {
  ...
  // 验证树的状态、节点边界及事件契约（具体场景见用例标题）。
  it('honors allowDrop and emits complete metadata without mutating input', () => createRoot(dispose => {
    disposers.push(dispose)
    const onDrop = vi.fn()
