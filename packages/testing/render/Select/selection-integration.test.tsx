...
it('[select.selection.integration] controlled rejection and parent update',()=>{
 const [value,set]=createSignal("a",{ownedWrite:true}),cb=vi.fn()
 const view=mount(()=><Select open virtual={false} value={value()} options={[{label:'A',value:'a'},{label:'B',value:'b'}]} onChange={cb}/>);dispose=view.dispose
