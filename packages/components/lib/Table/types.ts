...
import type { TableColumnDef, TableConfig, TableExpandable, TableIns, TableKey, TablePaginationConfig, TableRowSelection } from 'upthrust-competence'
...
export interface TableRef<T> {
  ...
  table: TableIns<T>
  scrollTo: (target: { key?: TableKey; index?: number; top?: number; align?: 'start' | 'center' | 'end' | 'nearest' }) => void
}
